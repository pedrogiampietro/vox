...
class StressClient {
  ...
  constructor(readonly nickname: string, url: string) {
...
async function main(): Promise<void> {
  ...
  console.log(`clientes=${options.clients} · speakers=${options.speakers} · duracao=${options.durationSec}s · voz=${options.voiceBytes}B/${options.voiceIntervalMs}ms`);
  ...
  const clients = Array.from({ length: options.clients }, (_, index) => (
    new StressClient(`stress-${runId}-${index + 1}`, options.url)
  ));
  ...
  try {
    ...
    const payload = new Uint8Array(options.voiceBytes);
    let sequence = 0;
    const voiceTimer = speakers.length > 0
      ? setInterval(() => {
          payload.fill(sequence & 0xff);
          for (const speaker of speakers) speaker.sendVoice(sequence, payload);
          sequence++;
        }, options.voiceIntervalMs)
      : null;
  ...
  printSummary(clients, adminRuntime);
  if (clients.length === 0 || clients.some((client) => !client.live)) process.exitCode = 1;
  if (failures.length > 0) process.exitCode = 1;
}

function printSummary(clients: StressClient[], adminRuntime: AdminRuntime | null): void {
  ...
  console.log(`  RTT: ${latencies.length > 0 ? `p50 ${formatMs(percentile(latencies, 0.50))} · p95 ${formatMs(percentile(latencies, 0.95))} · p99 ${formatMs(percentile(latencies, 0.99))}` : 'sem amostras'}`);
  if (adminRuntime) {
  ...
  for (const failure of failures.slice(0, 12)) console.log(`  falha: ${failure}`);
  if (failures.length > 12) console.log(`  ... mais ${failures.length - 12} falhas`);
  console.log(failures.length === 0 && live === clients.length ? '\nPASS' : '\nFAIL');
}

function parseOptions(): Options {
  if (process.argv.includes('--help') || process.argv.includes('-h')) {
    console.log('Uso: npm run stress -- [--clients N] [--speakers N] [--duration SEC] [--url WS_URL]');
    console.log('Env: STRESS_ADMIN_TOKEN, STRESS_ADMIN_URL, STRESS_CONFIRM=1, STRESS_VOICE_BYTES, STRESS_VOICE_INTERVAL_MS');
    process.exit(0);
  }
  return {
    ...
    adminUrl: argument('--admin-url') ?? process.env.STRESS_ADMIN_URL ?? adminUrlFor(DEFAULT_URL),
