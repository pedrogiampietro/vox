...
function renderRuntimeMetrics(runtime: RuntimeMetrics | null): HTMLElement {
  ...
  const voiceFanoutFrames = runtime.traffic.voiceFanoutFramesTotal ?? 0;
  const voiceFanoutRecipients = runtime.traffic.voiceFanoutRecipientsTotal ?? 0;
  ...
  details.append(
    ...
    text('span', 'mono subtle', `fan-out ${voiceFanoutRecipients.toLocaleString('pt-BR')} destinos em ${voiceFanoutFrames.toLocaleString('pt-BR')} pacotes`),
...
function renderRuntimeInsights(runtime: RuntimeMetrics): HTMLElement {
  ...
  for (const edge of runtime.edges ?? []) {
    ...
    row.append(
      ...
      text('span', 'mono subtle', `${edge.sessions} sessões · handshake ${handshakes.successRate.toFixed(1)}% · p50 ${formatMs(handshakes.p50Ms)} · p95 ${formatMs(handshakes.p95Ms)} · ${handshakes.cancelled} canceladas · ${edge.traffic.droppedPackets} drops`),
    );
...
function setGroup(server: ServerDetail, client: ClientInfo): void {
  const raw = prompt('Grupo: 0 visitante, 1 spy, 2 membro, 3 elite, 4 suporte, 5 moderador, 6 admin, 7 leader, 8 dono', String(client.group));
  const group = Number(raw);
  if (!Number.isInteger(group) || group < Group.Guest || group > Group.Dono) return;
